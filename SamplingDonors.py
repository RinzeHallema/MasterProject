...
def clean_dict_from_marg0(marg_lh: dict) -> dict:
    ...
    cleaned_dict = {}
    for locus in marg_lh[0]:  # donor 0 is major donor
        cleaned_dict[locus] = {}
...
def clean_dict_from_marg1(marg_lh: dict) -> dict:
    cleaned_dict = {}
    for locus in marg_lh[1]:  # donor 1 is the second donor
        cleaned_dict[locus] = {}
...
def clean_dict_from_marg2(marg_lh: dict) -> dict:
    cleaned_dict = {}
    for locus in marg_lh[2]:  # donor 2 is the third donor
        cleaned_dict[locus] = {}
...
def sample_full_genotypes(marg_lh: dict, loci_list: list, n_samples: int) -> list:
    #Sample genotypes with as weights the posterior probabilities
    posterior_dict = {}
...
def compute_full_LRs(marg_lh: dict, loci_list: list, sampled_genotypes: list) -> list:
    LR_per_locus = {}
    for locus in loci_list:
...
def make_posterior_plot(log10_lrs, posterior_probs, name_plot, true_LR, threshold = 0.5):
...
#
#    plt.tight_layout()
#    plt.savefig(f'plots/{name_plot}_posteriorscatter_cutoff.png')
#    plt.show()
#
...
all_loci = ['D1S1656', 'TPOX', 'D2S441', 'D2S1338', 'D3S1358', 'FGA', 'D5S818', 'CSF1PO', 'SE33', 'D7S820', 'D8S1179', 'D10S1248', 'TH01', 'vWA', 'D12S391', 'D13S317', 'PentaE', 'D16S539', 'D18S51', 'D19S433', 'PentaD', 'D21S11', 'D22S1045']

kit = all_loci                                                                                                  ### Choose a kit
llh_estimate, marg_likelihood_per_locus_genotype = dnax.run(Path("Mixtures/1_1D3.txt"), 3)          ### Choose a mixture/number of donors
cleanmarg = clean_dict_from_marg0(marg_likelihood_per_locus_genotype)                                           ### Choose donor (0 -> major donor)
sampled_genotypes, posterior_dict = sample_full_genotypes(cleanmarg, kit, 100000)                      ### Choose the number of sampled genotypes
full_LRs = compute_full_LRs(cleanmarg, kit, sampled_genotypes)
df = pd.read_csv(r'U:\RIHAL-admin\PycharmProjects\PythonProject\.venv\1A.csv', delimiter=';')     ### Choose the true donor
...
true_LR = compute_full_LRs(cleanmarg, kit, [true_genotype_withQ])
log_true_LR = math.log10(true_LR[0])
lr_values = list(full_LRs)
log10_lrs = np.log10(lr_values)
make_histogram(log10_lrs, '1_1D3_first_sampling', log_true_LR)                                         ### Choose a name to save the plot




#make_posterior_plot(log10_lrs, compute_full_genotype_posteriors(kit, sampled_genotypes, posterior_dict), '1_1D3_first_sampling', log_true_LR)

#Print genotype of the highest LR

# top = sorted(zip(sampled_genotypes, full_LRs), key=lambda x: x[1], reverse=True)
# for combo, lr in top[:1]:
#     print(f"{combo} => LR: {lr:.4e}")
# print(len(top))
